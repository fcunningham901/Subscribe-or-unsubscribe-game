...
app.get('/host', (req, res) => {
  // Use Render public URL if available, otherwise local IP
  const baseUrl = process.env.RENDER_EXTERNAL_URL
    ? process.env.RENDER_EXTERNAL_URL
    : `http://${getLocalIP()}:${server.address()?.port || PORT}`;
  res.send(HOST_HTML.replace('{{PLAYER_URL}}', `${baseUrl}/play`));
});
...
app.get('/qr', async (req, res) => {
  // Priority: RENDER_EXTERNAL_URL env var > request host header > local IP fallback
  let baseUrl;
  if (process.env.RENDER_EXTERNAL_URL) {
